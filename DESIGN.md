...
- **`.panel-header--flush`** — zero horizontal padding, for headers whose immediate scroll container already sits inside an outer `.panel`'s own padding (Transfer Detail, Manager Reply, Regional Director Override, and — combined with `--bleed-top-left` below — Transfers list). Adding the base class's own side padding on top of that would double the inset.
- **`.panel-header--bleed-corner` / `.panel-header--bleed-top-left`** — pull a header out to its `.panel` ancestor's *true* edge with a negative margin equal to the ambient padding it would otherwise sit inside, then let the header's own unmodified base padding (`--bleed-corner`) or explicitly-restored padding (`--bleed-top-left`, layered on top of `--flush`) push the text back to exactly where it was. Margin and padding cancel, so text position doesn't move — only the header's *box* does, out to the panel's rounded corner where `.panel`'s `overflow: hidden` can clip it to match. Without this, a header sitting ~24px inside its ancestor's own padding (Simulate Incident, Transfers list) reads as a separate floating rectangle pasted inside the panel rather than the panel's own top bar — a mismatch found live, by screenshot, against Network Status's genuinely-flush header (whose ancestor panel is zero-padding to begin with, #23/#30, so it never needed bleeding). `--bleed-corner` (top+left+right) is for Simulate Incident's header, which has no adjacent scrollbar to avoid. `--bleed-top-left` (top+left only) is for the Transfers list header, which shares its scroll container with a persistent scrollbar (#14/#25) on the right — bleeding that side too would put the header flush against the scrollbar thumb, so the right side keeps its existing container-supplied inset (`--flush`'s `padding-right: 0` stays in effect; `--bleed-top-left` only restores `padding-top`/`padding-left`).
...
**Gap below a header**: every panel header needs deliberate space before its immediate content — without it, the first row/filter/control sits flush against the header's `border-bottom` (found live, by screenshot). `.floating-panel--network__body`, `.transfer-list`, and `#sim-heading + .controls-row` each carry an explicit `margin-top: var(--space-2)` for this; it's on the *content* side rather than the header's own `margin-bottom` so it doesn't have to be undone for headers that don't want it (the on-ink console header's tighter 4px gap, `.detail-block__title`'s own 8px).
...
**Transfers list (#34 follow-up)**: the same scrollbar-runs-behind-the-header bug #30 found and fixed for Network Status also existed here — `.transfers-strip__list` was both the border-bottom-divided list region *and* its own scroll container, with the sticky `#transfers-heading` as its first child, so the scrollbar spanned the container's full height and visibly ran behind the header row. Fixed the same way: `.transfers-strip__list` is now a non-scrolling flex column holding the header (`.panel-header--static` — it no longer has a scrolling ancestor of its own to pin against) and `.transfers-strip__list-body`, a plain wrapper around `#transfer-list` that owns `overflow-y: auto` and the scrollbar styling `.transfers-strip__list` used to carry. Going static also sidesteps the sticky-padding-edge interaction noted above — `.panel-header--bleed-top-left` still handles the header's left-edge flush-to-corner bleed, but no longer needs any top-axis special-casing now that the header isn't inside a scroll container at all.
